...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Staff Sections</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="card card-success card-outline no-radius">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-plus"></i> Create Staff Section</h3>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('staff-sections.store') }}" method="post">
                                @csrf
                                <div class="row">
                                    <div class="form-group col-md-12">
                                        <label>Department Name</label>
                                        <select name="staff_department_id" class="form-control @error('staff_department_id') is-invalid @enderror no-radius">
                ...
                <div class="col-md-8">
                    <div class="card card-primary card-outline no-radius">
                        <div class="card-header">
                            <h3 class="card-title">Staff Sections List</h3>
                        </div>
                        <div class="card-body">
                            ...
                            @foreach($sections as $section)
                                <tr>
                                    <td>{{ $i }}</td>
                                    <td>{{ $section->staff_department->name }}</td>
                                    <td>{{ $section->name }}</td>
                                ...
                                <div class="modal fade" id="editSection{{ $section->id }}" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content no-radius">
                                            <form action="{{ route('staff-sections.update', $section->id) }}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                <div class="modal-body text-sm">
                                                    <div class="form-group col-md-12">
                                                        <label>Department Name</label>
                                                        <select name="staff_department_id" class="form-control @error('staff_department_id') is-invalid @enderror no-radius">
                                                            <option value="{{ $section->staff_department_id }}">{{ $section->staff_department->name }}</option>
                                                            @foreach($departments as $department)
                                                                <option value="{{ $department->id }}">{{ $department->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="form-group col-md-12">
                                                        <label>Section Name</label>
                                                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror no-radius" value="{{ old('name', $section->name) }}">
                                                        @if($errors->has('name'))
                                                            <small class="text-danger">{{ $errors->first('name') }}</small>
                                                        @endif
                                                    </div>
                                ...
                                <!-- Start Delete Modal -->
                                <div class="modal fade" id="deleteSection{{ $section->id }}" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">

                                    <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content no-radius">
                                            <form action="{{ route('staff-sections.destroy', $section->id) }}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                @method('DELETE')
                                                <div class="modal-header">
